...
CREATE INDEX IF NOT EXISTS idx_bpv_business ON public.business_page_views(business_id, viewed_at DESC);
CREATE INDEX IF NOT EXISTS idx_bpv_viewer ON public.business_page_views(viewer_id) WHERE viewer_id IS NOT NULL;

-- Anyone authenticated can insert (tracks their visit)
DROP POLICY IF EXISTS "Authenticated users can log page views" ON public.business_page_views;
...
DROP POLICY IF EXISTS "Business owners can read own page views" ON public.business_page_views;
...
-- Admins can read all
DROP POLICY IF EXISTS "Admins can read all page views" ON public.business_page_views;
