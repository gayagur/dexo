...
-- Indexes
create index if not exists idx_furniture_designs_customer on public.furniture_designs(customer_id);

-- RLS: customers can manage their own designs
drop policy if exists "Customer can read own designs" on public.furniture_designs;
create policy "Customer can read own designs"
  on public.furniture_designs for select
  using (auth.uid() = customer_id);

drop policy if exists "Customer can insert own designs" on public.furniture_designs;
create policy "Customer can insert own designs"
  on public.furniture_designs for insert
  with check (auth.uid() = customer_id);

drop policy if exists "Customer can update own designs" on public.furniture_designs;
create policy "Customer can update own designs"
  on public.furniture_designs for update
  using (auth.uid() = customer_id);

drop policy if exists "Customer can delete own designs" on public.furniture_designs;
...
alter table public.projects
  add column if not exists furniture_design_id uuid references public.furniture_designs(id) on delete set null;
